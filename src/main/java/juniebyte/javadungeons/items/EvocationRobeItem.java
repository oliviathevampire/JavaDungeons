...
public class EvocationRobeItem extends ArmorItem {
	...
	public EvocationRobeItem(String id, ArmorMaterial armorMaterial, EquipmentSlot slotType, Settings properties, boolean unique) {
		...
		builder.put(EntityAttributes.GENERIC_MOVEMENT_SPEED, new EntityAttributeModifier(uuid, "Armor speed boost",
				0.15D * 0.5D, EntityAttributeModifier.Operation.MULTIPLY_BASE));
	...
	public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
		...
		if (this.unique) {
			tooltip.add(new LiteralText(
					Formatting.WHITE + "" + Formatting.ITALIC + "The Ember Robe was created by Illager Evokers"));
			tooltip.add(new LiteralText(
					Formatting.WHITE + "" + Formatting.ITALIC + " to distinguish themselves from the common guard."));
			tooltip.add(new LiteralText(Formatting.GREEN + "Burns Nearby Enemies (Burning I)"));
		} else {
			tooltip.add(new LiteralText(
					Formatting.WHITE + "" + Formatting.ITALIC + "Potent magical runes are weaved into"));
			tooltip.add(new LiteralText(
					Formatting.WHITE + "" + Formatting.ITALIC + " the fabric of these robes, their"));
			tooltip.add(new LiteralText(
					Formatting.WHITE + "" + Formatting.ITALIC + " origins and powers are shrouded in mystery."));
		}
	}
