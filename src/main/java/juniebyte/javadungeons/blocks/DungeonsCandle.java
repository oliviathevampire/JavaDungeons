...
public class DungeonsCandle extends Block {
	...
	public DungeonsCandle(float hardness, float resistance, BlockSoundGroup sounds, DyeColor dyeColor, String type) {
		super(FabricBlockSettings.of(new FabricMaterialBuilder(dyeColor).destroyedByPiston().build()).strength(hardness, resistance).strength(hardness, resistance).sounds(sounds).nonOpaque().luminance(state ->
				state.get(LIT) ? 15 : 0));
	...
	public static void extinguish(@Nullable PlayerEntity player, BlockState state, WorldAccess world, BlockPos pos) {
		...
		if (state.getBlock() instanceof AbstractCandleBlock) {
			...
			world.addParticle(ParticleTypes.SMOKE,
					posX + 0.5D, posY + 0.5D, posZ + 0.5D, 0.0D, 0.10000000149011612D, 0.0D);
		}
			...
			if (f < 0.3F) {
				...
				if (f < 0.17F) {
					world.playSound(
							posX + 0.5D,
							posY + 0.5D,
							posZ + 0.5D, SoundEvents.BLOCK_CANDLE_AMBIENT, SoundCategory.BLOCKS,
